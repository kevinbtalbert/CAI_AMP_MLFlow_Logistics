...
--------------------------------------------------------
``MLFLOW_RUN_ID``   Load a specific training run artifact (recommended).
                    Copy the run ID from the MLflow UI after training.
If not set, the API falls back to the most-recent run across all experiments.
...
def _load_model() -> None:
    ...
    Resolution order:
    1. ``MLFLOW_RUN_ID`` — specific run artifact (recommended).
    2. Fallback          — most-recent run across all experiments.
    """
